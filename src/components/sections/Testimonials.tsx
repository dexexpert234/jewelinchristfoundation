    ...
    <section className="py-28 gradient-royal text-primary-foreground relative overflow-hidden">
      ...
      <div className="container relative z-10">
        <div className="text-center mb-14">
          <p className="text-primary-foreground/70 text-xs uppercase tracking-[0.4em] mb-4">Voices of Impact</p>
          <h2 className="font-display text-4xl md:text-5xl">Stories That <em className="text-primary-foreground not-italic font-semibold">Inspire</em></h2>
        </div>
        ...
        <div className="max-w-3xl mx-auto relative min-h-[320px]">
          {items.map((t, idx) => (
            <div key={idx} className={`absolute inset-0 transition-all duration-700 ${idx === i ? "opacity-100 translate-x-0" : idx < i ? "opacity-0 -translate-x-12" : "opacity-0 translate-x-12 pointer-events-none"}`}>
              <Quote className="h-16 w-16 text-primary-foreground/40 mx-auto mb-6" />
              <p className="font-display text-2xl md:text-3xl italic text-center leading-relaxed mb-8">
                "{t.text}"
              </p>
              <div className="flex items-center justify-center gap-4">
                <div className="h-14 w-14 rounded-full bg-primary-foreground flex items-center justify-center font-display text-xl text-background font-semibold">{t.initials}</div>
                <div>
              ...
              <div className="flex justify-center gap-1 mt-4">
                {Array.from({ length: t.rating }).map((_, k) => <Star key={k} className="h-4 w-4 fill-primary-foreground text-primary-foreground" />)}
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-center gap-2 mt-12">
          {items.map((_, idx) => (
            <button key={idx} onClick={() => setI(idx)} aria-label={`Testimonial ${idx + 1}`} className={`h-1.5 rounded-full transition-smooth ${idx === i ? "w-10 bg-primary-foreground" : "w-5 bg-primary-foreground/30"}`} />
          ))}
        </div>
      </div>
    </section>
